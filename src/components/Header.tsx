        ...
        >
            ...
            <div className="w-full relative z-50">
                <div className="max-w-[1400px] mx-auto flex items-center justify-between px-4 lg:px-6 h-[96px] relative">
                    ...
                    {/* Desktop Menu */}
                    <nav
                        className={`hidden lg:flex items-center h-full gap-8 text-[14px] font-medium tracking-wider uppercase ${isHomePage && !isScrolled ? "text-white" : "text-[var(--text-primary)]"
                            }`}
                    >
                        {MENU_ITEMS.map((item, index) => (
                            ...
                            >
                                ...
                                {/* Mega Menu (Your original design) */}
                               {/* Mega Menu (Height reduced + full images visible) */}
{item.dropdown && activeMenu === item.slug && (
  <div className="fixed left-0 right-0 top-[122px] bg-white text-[var(--text-primary)] shadow-xl pt-10 pb-12 border-t border-gray-200 animate-dropdown z-40">
    <div className="mx-auto max-w-[1400px] px-10 grid grid-cols-4 gap-12">
      ...
      {/* Banner Section — full image visible */}
          ...
          >
            <div className="relative w-full h-full flex items-center justify-center">
              ...
              />
                    ...
                    {/* Right Section (Icons) */}
                    <div
                        className={`flex items-center gap-4 ${isHomePage && !isScrolled ? "text-white" : "text-[var(--text-primary)]"
                            }`}
                    >
                        ...
                        <div className="flex lg:hidden gap-3">
                            {[User, Heart, ShoppingBag].map((Icon, i) => (
                                <Icon key={i} size={22} className="cursor-pointer" />
                            ))}
                        </div>
                    </div>
                </div>
...
{isOpen && (
  <div className="fixed inset-0 z-50 flex lg:hidden">
    ...
    <div className="relative w-[80%] max-w-[320px] h-[100vh] bg-white shadow-xl animate-slideIn flex flex-col">
        ...
        {MENU_ITEMS.map((item, index) => (
          ...
          >
            ...
            {item.dropdown && (
              <button
                onClick={() =>
                  setActiveMenu(activeMenu === item.slug ? null : item.slug)
                }
                className="text-[var(--text-primary)] hover:text-[var(--brand-primary)] transition"
              >
                {activeMenu === item.slug ? <X size={18} /> : <Menu size={18} />}
              </button>
            )}
          </div>
        ))}
      ...
      {/* Submenu Drawer */}
      {activeMenu && (
        <div className="absolute top-0 left-0 w-full h-[100vh] bg-white z-50 animate-slideIn flex flex-col">
          {/* Submenu Header */}
          <div className="flex items-center p-4 border-b border-gray-200">
            <button
              onClick={() => setActiveMenu(null)}
              className="flex items-center gap-2 text-[var(--brand-primary)] uppercase font-medium"
            >
              <X size={18} /> Back
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-4">
            <h3 className="text-lg font-semibold mb-4">
              {MENU_ITEMS.find((m) => m.slug === activeMenu)?.label}
            </h3>
            <div className="flex flex-col space-y-3 text-sm font-medium">
              {MENU_ITEMS.find((m) => m.slug === activeMenu)?.dropdown?.left.map(
                (link, i) => (
                  <Link
                    href="#"
                    key={i}
