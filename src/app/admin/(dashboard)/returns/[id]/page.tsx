...
import { returnRequestAdminService } from "@/lib/services/returnRequestService";
...
export default function ReturnRequestDetailPage() {
    ...
    const fetchRequest = async () => {
        setLoading(true);
        const res = await returnRequestAdminService.getById(token!, Number(id));
        if (res.success) {
            setReqData(res.data);
    ...
    const updateStatus = async (status: string) => {
        const res = await returnRequestAdminService.updateStatus(token!, Number(id), {
            status,
            adminNote,
            refundAmount,
        });
