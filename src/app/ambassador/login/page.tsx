...
export default function AmbassadorLoginPage() {
  ...
  // Redirect if already logged in (ambassadors carry the "affiliate" user role)
  useEffect(() => {
    if (status !== "authenticated") return;
    if (session?.user?.role === "affiliate") {
      router.replace(ROUTES.AMBASSADOR.DASHBOARD);
    }
  }, [status, session, router]);

  useEffect(() => {
    const rememberedEmail = loadWithExpiry("gaza_arabia_remember_ambassador");
    if (rememberedEmail) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response: any = await signIn("credentials", {
        ...
        role: "affiliate", // ambassadors authenticate under the affiliate role
      });
      ...
      if (form.rememberMe) {
        saveWithExpiry("gaza_arabia_remember_ambassador", form.email);
      } else {
        localStorage.removeItem("gaza_arabia_remember_ambassador");
      }
      ...
      setTimeout(() => {
        router.push(ROUTES.AMBASSADOR.DASHBOARD);
      }, 1000);
    } catch (err: any) {
      setAlert({
        isOpen: true,
      ...
      <div className="w-full max-w-md mx-4 z-10">
        <div className="bg-white shadow-2xl rounded-2xl overflow-hidden border border-gray-100">
          ...
          <div className="px-6 py-8 text-center bg-gradient-to-r from-[var(--brand-primary)] to-[var(--brand-secondary)] text-white">
            ...
            <h1 className="text-2xl font-bold">Ambassador Login</h1>
            <p className="text-sm opacity-90 mt-1">Access your ambassador dashboard</p>
          </div>
          ...
          <form onSubmit={onSubmit} className="px-6 py-8 space-y-6 bg-white">
            ...
            <p className="text-xs text-center text-gray-500">
              Don&rsquo;t have an ambassador account?{" "}
              <a href={ROUTES.AMBASSADOR.REGISTER} className="text-[var(--brand-primary)] hover:underline font-medium">
                Register here
              </a>
            </p>
            ...
            <p className="text-xs text-center text-gray-400">
              ...
              <a href={ROUTES.AFFILIATE.LOGIN} className="text-[var(--brand-primary)] hover:underline font-medium">
                Affiliate login
              </a>
            </p>
          </form>
